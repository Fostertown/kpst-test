...
/* PART 6: Grab the Paid Source (from domainRefSource) and store it in paidSource */
  //If rsQueryStringValue is not any of the email query strings values (in arrays, -1 is false, >= 0 is true), and rsRefSource is a Paid Source, store in paidSource
  if ($.inArray(rsQueryStringValue, ['css', 'nl', 'nu', 're', 'sa', 'surv']) == -1 ) {
      if (rsRefSource == "AdRoll") {paidSource = 'AdRoll';}
      else if (rsRefSource == "BingAds") {paidSource = 'BingAds';}
      else if (rsRefSource == "Facebook Paid") {paidSource = 'Facebook Paid';}
      else if (rsRefSource == "Google AdWords") {paidSource = 'Google AdWords';}
      else if (rsRefSource == "LinkedIn Paid") {paidSource = 'LinkedIn Paid';}
      else if (rsRefSource == "Twitter Paid") {paidSource = 'Twitter Paid';}
  }

/* PART 7: Assign the direct referral source, in refSource, with a picklist value. rs= always wins for direct */
  if (rsRefSource != null) {
  ...
  } else if (paidSource != null) {
    refSource = paidSource;
  } else {
    refSource = null;
  }
